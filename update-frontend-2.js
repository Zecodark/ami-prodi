...
// We will find the boundaries for our replacement.
// Boundary 1: from `const handleNodeClick` up to `// ===================================================================` (Main JSX)
// We'll replace the handlers.
  ...
  const handleSubmit = async (isDraft: boolean = false) => {
    ...
    try {
      ...
      for (const form of isianList) {
        // Skip valid items unless it's a new draft or proses over an old revision? 
        // We probably shouldn't submit valid items again, but backend handles it or we filter here.
...
const startIndex = code.indexOf('  const handleNodeClick = async');
const endIndex = code.indexOf('  // ===================================================================\n  // Tree renderer');

if (startIndex !== -1 && endIndex !== -1) {
  code = code.substring(0, startIndex) + handlerReplacement + code.substring(endIndex);
}
