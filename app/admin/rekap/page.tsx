...
export default function AdminRekapPage() {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        // Ambil list prodis
        const prRes = await fetch('/api/prodis', { headers });
        const prJson = await prRes.json();
        if (prJson.data?.length > 0) {
          setProdis(prJson.data);
          setSelectedProdiId(String(prJson.data[0].id));
        }
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Ambil status map
        await fetchStatusMap(selectedProdiId, selectedPeriodeId);
      } catch (e) {
        console.error(e);
      } finally {
        setLoading(false);
  ...
  if (loading && (!periodes.length || !prodis.length)) {
    return (
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Pantau seluruh isian AMI dari semua prodi secara hierarki (View Only).
          </p>
        </div>
        <button 
           ...
           <div>
              <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => fetchStatusMap(selectedProdiId, selectedPeriodeId)}
